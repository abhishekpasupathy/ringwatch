...
export default function AccountLookupBar({ onAccountFocus }: AccountLookupBarProps) {
  ...
  async function handleSearch(targetId?: string) {
    ...
    try {
      ...
      setResult(data);
      if (data.found && onAccountFocus) onAccountFocus(searchId);
    } catch {
      setResult({
        found: false,
        accountId: searchId,
  ...
  return (
    <div className="bg-[#0b0f19]/95 border-b border-white/10 p-4">
        ...
        <div className="flex-1 space-y-2">
          <div className="flex items-center gap-2">
            <div className="relative flex-1">
              ...
              />
              {query && (
                <button onClick={() => { setQuery(""); setResult(null); }} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 hover:text-white">
                  <X size={15} />
                </button>
              )}
            </div>
            ...
            >
              {loading ? <div className="w-4 h-4 border-2 border-slate-950 border-t-transparent rounded-full animate-spin" /> : <><span>Inspect</span><ArrowRight size={14} /></>}
            </button>
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {DEMO_CHIP_CONFIG.map((chip) => {
              ...
              return (
                <button
                  key={chip.key}
                  onClick={() => { setQuery(accountId); handleSearch(accountId); }}
      ...
      {loading && <div className="max-w-6xl mx-auto mt-3 h-20 bg-[#131b2e] border border-slate-800 rounded-xl animate-shimmer" />}
        ...
        <div className="max-w-6xl mx-auto mt-3 transition-all duration-300">
          {result.status === "NOT_FOUND" && (
            <div className="bg-[#141b2d] border border-slate-700/60 rounded-xl p-4 flex items-start gap-3">
              ...
              <div><h4 className="text-slate-200 font-bold text-sm">Account Not Found</h4><p className="text-slate-400 text-xs mt-1 font-mono">{result.message}</p></div>
            </div>
          )}
          ...
          {result.status === "SAFE" && (
            <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-xl p-4 flex items-start gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <span className="bg-emerald-500/20 text-emerald-400 text-xs font-bold px-2.5 py-0.5 rounded-full uppercase tracking-wider border border-emerald-500/30">Safe Account</span>
          ...
          {(result.status === "RING_MEMBER" || result.status === "EXPOSED_MERCHANT") && (
            <div className={`rounded-xl p-4 border glass-panel ${result.status === "RING_MEMBER" ? "bg-red-500/10 border-red-500/40" : "bg-amber-500/10 border-amber-500/40"}`}>
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-2">
                  <AlertTriangle size={20} className={result.status === "RING_MEMBER" ? "text-red-400" : "text-amber-400"} />
                  <span className={`text-xs font-bold uppercase tracking-wider px-2.5 py-0.5 rounded-full ${result.status === "RING_MEMBER" ? "bg-red-500/20 text-red-400 border border-red-500/30" : "bg-amber-500/20 text-amber-400 border border-amber-500/30"}`}>{result.statusLabel}</span>
                  <span className="text-slate-300 text-xs font-mono font-bold">ID: {result.accountId}</span>
                  {result.clusterId !== undefined && <span className="text-slate-400 text-xs font-mono">· Cluster #{result.clusterId}</span>}
                </div>
                {result.suspicionTier && <span className="text-xs font-bold text-slate-300 border border-slate-700 bg-slate-900 px-2.5 py-0.5 rounded-full">Tier: {result.suspicionTier}</span>}
              </div>
              ...
              {result.explanation && (
                <div className="mt-3 bg-[#0a0e17] border border-slate-800 rounded-xl p-3.5">
                  <div className="flex items-center gap-1.5 text-xs text-teal-400 font-bold mb-1"><Sparkles size={14} /> AI Analysis Summary (Groq)</div>
                  <p className="text-slate-300 text-xs leading-relaxed font-sans">{result.explanation}</p>
                </div>
              )}
              {result.structuralEvidence && (
                <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                  <div className="bg-[#0e131f] p-2.5 rounded-lg border border-slate-800 text-center"><div className="text-slate-400 text-[10px] uppercase font-semibold">Cluster Size</div><div className="text-white font-bold font-mono text-sm">{result.structuralEvidence.clusterSize} accounts</div></div>
                  <div className="bg-[#0e131f] p-2.5 rounded-lg border border-slate-800 text-center"><div className="text-slate-400 text-[10px] uppercase font-semibold">Exposed Merchants</div><div className="text-amber-400 font-bold font-mono text-sm">{result.structuralEvidence.exposedMerchants} accounts</div></div>
                  <div className="bg-[#0e131f] p-2.5 rounded-lg border border-slate-800 text-center"><div className="text-slate-400 text-[10px] uppercase font-semibold">Density</div><div className="text-teal-400 font-bold font-mono text-sm">{result.structuralEvidence.internalEdgeDensity}</div></div>
                  <div className="bg-[#0e131f] p-2.5 rounded-lg border border-slate-800 text-center"><div className="text-slate-400 text-[10px] uppercase font-semibold">Time Bursts</div><div className={result.structuralEvidence.timeBurstPresent ? "text-red-400 font-bold font-mono text-sm" : "text-emerald-400 font-bold font-mono text-sm"}>{result.structuralEvidence.timeBurstPresent ? "DETECTED" : "NONE"}</div></div>
                </div>
              )}
            </div>
          )}
